...
export class App {
  ...
  static renderDashboard() {
    ...
    const todayCourses = COURSES_DATA.filter(
      (c) => c.jour === todayName && c.start,
    );
    ...
    const totalHours = todayCourses.reduce(
      (a, c) => a + (timeToMin(c.end) - timeToMin(c.start)) / 60,
      0,
    );
  ...
  static renderCourseList() {
    ...
    el.innerHTML = COURSES_DATA.map((c) => {
      const color = courseColor(c);
      return `<div class="course-item" onclick="App.showCourseDetail('${c.code}')">
        ...
        <div class="ci-info">
          ...
          <div class="ci-meta">
            ...
            ${c.prof ? `<span style="display:inline-flex;align-items:center;gap:4px"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>${esc(c.prof)}</span>` : ""}
            ${
              c.jour
                ...
                : `<span style="display:inline-flex;align-items:center;gap:4px;color:var(--red)"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--red)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>Horaire non défini</span>`
            }
          </div>
        </div>
  ...
  static renderScheduleList() {
    ...
    COURSES_DATA.filter((c) => c.jour).forEach(
      (c) => byDay[c.jour] && byDay[c.jour].push(c),
    );
      ...
      .map(([day, courses]) => {
        ...
        return `<div class="card mb-4">
          <div class="card-header">
            ...
            <div class="tag tag-green">${courses.length} cours</div>
          </div>
  ...
  static showCourseDetail(code) {
    ...
    document.getElementById("cdBody").innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
        <div class="tip-card" style="padding:12px">
          <div style="font-size:10px;color:var(--muted);margin-bottom:4px">CODE UE</div>
          <div style="font-family:'Syne',sans-serif;font-weight:700;color:${color}">${esc(c.code)}</div>
        </div>
        <div class="tip-card" style="padding:12px">
          <div style="font-size:10px;color:var(--muted);margin-bottom:4px">CRÉDITS</div>
          <div style="font-family:'Syne',sans-serif;font-weight:700;color:var(--text)">${c.credits} ECTS</div>
        </div>
      </div>
  ...
  static saveStudySession() {
    ...
    ["studyCourse", "studyDate", "studyDuration"].forEach((id) =>
      document.getElementById("err-" + id).classList.remove("show"),
    );
    if (!course) {
      document.getElementById("err-studyCourse").classList.add("show");
      valid = false;
    }
    if (!date) {
      document.getElementById("err-studyDate").classList.add("show");
      valid = false;
    }
    if (!dur || dur < 0.5 || dur > 8) {
      document.getElementById("err-studyDuration").classList.add("show");
      valid = false;
    }
    if (!valid) return;
          ...
          return `<div class="session-item">
            ...
            <div class="session-info">
              ...
              <div class="session-when">
                ${formatDate(s.date)}${s.startTime ? ` · ${s.startTime}` : ""} · ${s.duration}h ·
      ...
      statsEl.innerHTML = Object.entries(byCourse)
        .map(([code, hrs]) => {
          ...
          return `<div style="margin-bottom:10px">
            ...
            <div class="bar-track">
              <div class="bar-fill" style="width:${(hrs / max) * 100}%;background:${color}"></div>
            </div>
          </div>`;
        })
        .join("");
