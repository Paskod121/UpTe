...
export class App {
  ...
  static showCourseDetail(code) {
    ...
    document.getElementById("cdBody").innerHTML = `
      ...
      ${c.jour ? `<div class="form-group"><div class="form-label">HORAIRE</div><div style="font-size:13px;color:var(--text)">${esc(c.jour)} · ${esc(c.start)} – ${esc(c.end)}</div></div>` : `<div style="color:var(--red);font-size:12px;margin-bottom:12px">⚠ Horaire non encore défini</div>`}
