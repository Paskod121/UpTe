...
export class UI {
  ...
  static cycleTheme() {
    ...
    // Toast premium avec icône thème
    const ICONS = {
      ...
      blue: `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>`,
      green: `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M7 12.5c1.5-3 7.5-3 9 0"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>`,
    };
    const LABELS = {
      light: "Thème Clair",
      blue: "Thème Bleu",
    ...
    // Supprimer les anciens toasts de thème
    document.querySelectorAll(".toast-theme").forEach((t) => {
    ...
    el.className = "toast success toast-theme";
    el.style.cssText =
      "min-width:220px;display:flex;align-items:center;gap:10px";
    el.innerHTML = `
      <span class="toast-icon">${ICONS[next.id]}</span>
      <span style="flex:1;font-weight:500">${LABELS[next.id]}</span>
      <div class="theme-preview-dots">
        ${THEMES.map((t) => `<span class="tpd ${t.id === next.id ? "active" : ""}" data-theme="${t.id}"></span>`).join("")}
      </div>`;
    tc.appendChild(el);
